...
export const PersonalPage = () => {
  ...
  const { cats, dogs } = useAppSelector((state) => state.animals);
  ...
  const changePhoto = (newIndex: number) => {
    if (timerForChange.current) {
      clearTimeout(timerForChange.current);
    }

    setIsChanging(true);
    timerForChange.current = window.setTimeout(() => {
      setIsChanging(false);
  ...
  useEffect(() => {
    ...
    if (pet && pet.sex === "Дівчинка") {
      ...
      setSterilized(
        pet.sterilized === true ? "Стерилізована" : "Не стерилізована"
      );
    } else if (pet && pet.sex === "Хлопчик") {
      setVaccinated(
        pet.vaccinated === true ? "Вакцинований" : "Не вакцинований"
      );
      setSterilized(
        pet.sterilized === true ? "Стерилізований" : "Не стерилізований"
      );
    }
  }, [pet]);
  ...
  useEffect(() => {
    ...
    if (petId && category) {
      const fetchPromise = getAnimalById(category, petId)
        .then((data) => {
          setPet(data); /*setMainPhoto(data.photo[0]) */
        })
  ...
  const transformValue = mainPhoto > 2 ? photoWidth + gap : 0;

  const rightSlide = () => {
    changePhoto(mainPhoto >= photo.length - 1 ? 0 : mainPhoto + 1);
  };

  const leftSlide = () => {
    changePhoto(mainPhoto === 0 ? photo.length - 1 : mainPhoto - 1);
  };
        ...
        <div className="personal__left">
          <img
            ...
            className={classNames("personal__photo personal__photo-main", {
              "personal__photo-disabled": isChanging,
            })}
          />
                    ...
                    <li key={image}>
                      <img
                        ...
                        className={classNames(
                          "personal__photo personal__photo-small",
                          {
                            "personal__photo-small--active":
                              index === mainPhoto,
                          }
                        )}
                      />
